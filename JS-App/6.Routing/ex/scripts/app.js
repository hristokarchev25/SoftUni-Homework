                ...
                .then(async (response) => {
                    firebase.auth().currentUser.getIdToken()
                        .then(async token => {
                            ...
                            let data = await (await sendRequest(`https://my-movies-7493e.firebaseio.com/userInfo/${response.user.uid}.json`,
                                token,
                                'GET'
                            )).json();
        ...
        if (!Object.values(data).includes('') && data.password === data.repeatPassword) {
            firebase.auth().createUserWithEmailAndPassword(data.username, data.password)
                .then((response) => {
                    ...
                    firebase.auth().currentUser.getIdToken()
                        .then(token => {
                            sendRequest(`https://my-movies-7493e.firebaseio.com/userInfo/${response.user.uid}.json`,
                                token,
                                'PUT',
                                JSON.stringify({
                                    hasNoTeam: true,
...
async function createViewHandler() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (!Object.values(data).includes('')) {
            let responseTeamKey = await (await sendRequest('https://my-movies-7493e.firebaseio.com/teams.json',
            ...
            await sendRequest(`https://my-movies-7493e.firebaseio.com/userInfo/${sessionStorage.getItem('uid')}.json`,
                sessionStorage.getItem('token'),
                'PUT',
...
async function getTeamsFromDB() {
    let response = await sendRequest('https://my-movies-7493e.firebaseio.com/teams.json', sessionStorage.getItem('token'), 'GET');
...
async function detailsViewHandler() {
    ...
    let team = await (await sendRequest(`https://my-movies-7493e.firebaseio.com/teams/-${document.URL.split(':-')[1]}.json`,
        sessionStorage.getItem('token'),
        'GET'
    )).json();
    this.name = team.name;
    ...
    let user = await (await sendRequest(`https://my-movies-7493e.firebaseio.com/userInfo/${sessionStorage.getItem('uid')}.json`,
        sessionStorage.getItem('token'),
        'GET'
    )).json();
...
async function leaveTeamHandler() {
    let team = await (await sendRequest(`https://my-movies-7493e.firebaseio.com/teams/${sessionStorage.getItem('teamId')}.json`,
        sessionStorage.getItem('token'),
        'GET'
    )).json();
    team.members === undefined ? team.members = [] : team.members = team.members;
    await sendRequest(`https://my-movies-7493e.firebaseio.com/teams/${sessionStorage.getItem('teamId')}.json`,
        sessionStorage.getItem('token'),
        'PATCH',
